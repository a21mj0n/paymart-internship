...
export default {
    ...
    methods:{
      async editProduct(){
          await axios.put(`https://marketpaymart.herokuapp.com/api/dashboard/product${this.$route.params.id}`, this.product)
          this.$router.push({name: "admin.products"})
       }
        },
    async created(){
          const {data} = await axios.get(`https://marketpaymart.herokuapp.com/api/dashboard/product${this.$route.params.id}`)
          this.product = data
